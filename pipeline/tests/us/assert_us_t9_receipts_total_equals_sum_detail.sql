-- MTS Table 9 self-check (API-RECON §D.3.2): published receipts T-row
-- (sequence 1.8) must equal Σ(D|RSG detail rows) per record_date, for all
...
WITH detail AS (
    ...
    WHERE data_type_cd = 'D' AND record_type_cd = 'RSG'
...
published AS (
    ...
    FROM {{ ref('stg_us_mts_table_9') }}
    WHERE data_type_cd = 'T' AND section = 'receipts'
)
