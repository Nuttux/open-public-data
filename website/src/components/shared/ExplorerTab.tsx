      ...
      <div className="flex items-center justify-between gap-3 mb-4">
        ...
        {hasToggle && (
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-500 hidden sm:inline">{tr('view.visualization')} :</span>
            <div className="flex bg-slate-800 rounded-lg border border-slate-700 p-0.5">
              ...
              {hasArr && (
                ...
                >
                  <ArrIcon /> <span className="hidden sm:inline">{tr('view.arrondissements')}</span><span className="sm:hidden">{tr('view.arrondissements_short')}</span>
                </button>
              )}
            </div>
          </div>
        ...
        <>
          ...
          >
            <div className="flex items-center gap-2">
              ...
              <span>
                {activeFilterCount > 0
                  ? `${tr('common.filters')} (${activeFilterCount} ${activeFilterCount > 1 ? tr('common.actives') : tr('common.active')})`
                  : `${tr('common.filter')} ${filterLabel}`}
              </span>
            </div>
