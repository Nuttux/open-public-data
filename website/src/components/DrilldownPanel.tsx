    ...
    >
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-3">
            ...
            >
              ...
              <span>{currentLevel > 0 ? t('common.back') : t('common.close')}</span>
            </button>
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            >
              <span style={{ color: barColor }}>
                {category === 'revenue' ? t('budget.revenue') : t('budget.expense')}
              </span>
            </div>
      ...
      <div className="rounded-lg p-4 mb-4" style={bgStyle}>
        <div className="flex items-center justify-between">
          <div>
            <span className="text-slate-300 font-medium">{t('common.total')}</span>
            <p className="text-sm text-slate-500">
              {filteredItems.length} {t('budget.posts')}
              {selectedSection !== 'all' && (
            ...
            <div className="flex-1 space-y-2">
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className={`text-sm font-medium ${sectionTotals.fonctionnement > 0 ? 'text-slate-200' : 'text-slate-500'}`}>{t('budget.operating')}</span>
                    <span className={`text-[10px] block ${sectionTotals.fonctionnement > 0 ? 'text-slate-400' : 'text-slate-600'}`}>{t('budget.operating_desc')}</span>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className={`text-sm font-medium ${sectionTotals.investissement > 0 ? 'text-slate-200' : 'text-slate-500'}`}>{t('budget.capital')}</span>
                    <span className={`text-[10px] block ${sectionTotals.investissement > 0 ? 'text-slate-400' : 'text-slate-600'}`}>{t('budget.capital_desc')}</span>
                  </div>
                </div>
                <div className="text-right">
      ...
      {canDrillDown && (
        <div className="flex items-center gap-2 mb-3 px-3 py-2 bg-slate-700/30 rounded-lg text-sm">
          <span className="text-slate-400">
            {isMobile ? `${t('budget.tap_bar_explore')} →` : `${t('budget.click_bar_explore')} →`}
          </span>
        </div>
      )}
      ...
      {displayItems.length > 0 ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 text-center">
          <p className="text-slate-400">{t('budget.no_expenses')}</p>
          <button
            onClick={() => setSelectedSection('all')}
