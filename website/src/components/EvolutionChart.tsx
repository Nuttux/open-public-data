  ...
  const option: EChartsOption = useMemo(() => ({
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        html += year;
        if (isVoted) {
          html += `<span style="font-weight: 400; font-size: 10px; color: #94a3b8; margin-left: 6px; vertical-align: middle; border: 1px solid #475569; border-radius: 3px; padding: 1px 4px;">${t('evolution.voted_label')}</span>`;
        }
        html += `</div>`;
        
        // Afficher Recettes et Dépenses (exclure Déficit car affiché séparément)
        items
          .filter(item => item.seriesName !== t('evolution.deficit'))
          .forEach(item => {
            html += `
        ...
        if (yearData) {
          ...
          const soldeLabel = yearData.solde >= 0 ? t('evolution.surplus') : t('evolution.deficit');
          html += `
    ...
    legend: {
      data: [t('evolution.revenue'), t('evolution.expenses'), t('evolution.deficit')],
