...
// BREAKDOWNS moved to useMemo inside component for i18n
...
function getGroupKey(m: MarchePublic, dim: string, t?: (k: string) => string): string {
  const notSpecified = t ? t('marches_annuel.not_specified') : 'Non renseigné';
  const other = t ? t('marches_annuel.other') : 'Autre';
  switch (dim) {
    case 'nature':
      return m.nature || notSpecified;
    case 'categorie':
      return m.categorie_libelle || notSpecified;
    default:
      return other;
  }
}
...
function getColumns(t: (k: string) => string): TableColumnDef<MarchePublic>[] { return [
  {
    key: 'objet', label: t('marches_annuel.contract'), align: 'left',
    render: (m, i) => (
  ...
  {
    key: 'fournisseur', label: t('marches_annuel.supplier'), hideOnMobile: true, align: 'left',
  ...
  {
    key: 'montant', label: t('marches_annuel.max_amount'), align: 'right',
...
export default function MarchesAnnuelTab({
  ...
  return (
    <AnnuelTab
      ...
      getGroupKey={(m, dim) => getGroupKey(m, dim, t)}
      ...
      itemLabel={t('marches.item_label')}
      columns={columns}
      ...
      kpiCards={stats && (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4 mb-6">
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 shadow-sm p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wide">{t('marches.total_amount')}</p>
            <p className="text-2xl font-bold text-slate-100 mt-1">{formatEuroCompact(stats.totalEnveloppe)}</p>
            <p className="text-xs text-slate-400 mt-1">{t('marches.contracts_notified').replace('{n}', formatNumber(stats.total))}</p>
          </div>
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 shadow-sm p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wide">{t('marches.median_amount')}</p>
            <p className="text-2xl font-bold text-teal-400 mt-1">{formatEuroCompact(stats.median)}</p>
            <p className="text-xs text-slate-400 mt-1">{t('marches.per_contract')}</p>
          </div>
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 shadow-sm p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wide">{t('marches.avg_duration')}</p>
            <p className="text-2xl font-bold text-teal-400 mt-1">{stats.dureeMoyenne ? formatDuration(stats.dureeMoyenne) : '—'}</p>
            <p className="text-xs text-slate-400 mt-1">{t('marches.per_contract')}</p>
          </div>
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 shadow-sm p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wide">{t('marches.unique_companies')}</p>
            <p className="text-2xl font-bold text-emerald-400 mt-1">{formatNumber(stats.fournisseursUniques)}</p>
            <p className="text-xs text-slate-400 mt-1">{t('marches.distinct_suppliers')}</p>
          </div>
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 shadow-sm p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wide">{t('marches.shared_contracts')}</p>
            <p className="text-2xl font-bold text-amber-400 mt-1">{stats.tauxMultiAttr.toFixed(0)}%</p>
            <p className="text-xs text-slate-400 mt-1">{t('marches.with_multiple')}</p>
          </div>
          <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 shadow-sm p-4">
            <p className="text-xs text-slate-400 uppercase tracking-wide">{t('marches.top_supplier')}</p>
