  ...
  const option: EChartsOption = useMemo(() => ({
    ...
    tooltip: {
      ...
      formatter: (params: unknown) => {
        ...
        html += d.year;
        if (d.estimated) {
          html += `<span style="font-weight: 400; font-size: 10px; color: #94a3b8; margin-left: 6px; vertical-align: middle; border: 1px solid #475569; border-radius: 3px; padding: 1px 4px;">${t('debt.tooltip_estimated')}</span>`;
        }
        html += `</div>`;
        ...
        if (d.emprunts != null || d.remboursement_principal != null) {
          ...
          if (d.emprunts != null) {
            html += `${t('debt.tooltip_loans')} : +${formatEuroCompact(d.emprunts)}`;
          }
          if (d.remboursement_principal != null) {
            html += ` · ${t('debt.tooltip_repayment')} : −${formatEuroCompact(d.remboursement_principal)}`;
          }
          html += `</div>`;
        }
        ...
        if (d.estimated) {
          ...
          html += t('debt.tooltip_estimation_detail');
          html += `</div>`;
        }
