      ...
      {isLoading && (
        <div className="absolute inset-0 bg-slate-900/50 backdrop-blur-sm z-[1000] flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-slate-300 text-sm">{t('map.loading_data')}</p>
          </div>
        </div>
      )}
            ...
            <Popup>
              <div className="min-w-[220px]">
                ...
                <div className="text-xs text-slate-600 space-y-1">
                  <p>
                    <strong>{t('map.popup.direction')}</strong>{' '}
                  ...
                  <p><strong>{t('map.popup.nature')}</strong> {sub.nature}</p>
                  <p><strong>{t('map.popup.objet')}</strong> {sub.objet}</p>
                  {sub.adresse && <p><strong>{t('map.popup.adresse')}</strong> {sub.adresse}</p>}
                  <p><strong>{t('map.popup.annee')}</strong> {sub.annee}</p>
                </div>
              </div>
            </Popup>
          </CircleMarker>
            ...
            <Popup>
              <div className="min-w-[200px]">
                ...
                <p className="text-2xl font-bold text-emerald-600 mb-2">
                  {log.nbLogements} {t('map.popup.logements')}
                </p>
                <div className="text-xs text-slate-600 space-y-1">
                  <p><strong>{t('map.popup.bailleur')}</strong> {log.bailleur}</p>
                  <p><strong>{t('map.popup.type')}</strong> {log.natureProgramme}</p>
                  <p><strong>{t('map.popup.mode')}</strong> {log.modeRealisation}</p>
                  <p><strong>{t('map.popup.annee')}</strong> {log.annee}</p>
                  <p><strong>{t('map.popup.arrondissement')}</strong> {log.arrondissement}ème</p>
                  <div className="mt-2 pt-2 border-t border-slate-700/50">
                    <p>{t('map.popup.plai')} {log.nbPLAI}</p>
                    <p>{t('map.popup.plus')} {log.nbPLUS}</p>
                    <p>{t('map.popup.pls')} {log.nbPLS}</p>
                  </div>
                </div>
              </div>
            </Popup>
            ...
            <Popup maxWidth={350}>
              <div className="min-w-[280px] max-w-[320px]">
                ...
                <div className="text-xs text-slate-600 space-y-1.5">
                  ...
                  <p><strong>{t('map.popup.annee')}</strong> {ap.annee}</p>
                    ...
                    {ap.isPrecise ? (
                      ...
                      <>
                        ...
                        <p className="text-slate-500 ml-5">
                          {ap.arrondissement === 0 ? t('map.popup.paris_centre') : `${ap.arrondissement}${t('map.popup.arrondissement_suffix')}`}
                        </p>
                      </>
                    )}
                  </div>
          ...
          <div className="space-y-1.5 text-xs">
            {showSubventions && (
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-400">{t('map.subventions')} ({geoSubventions.length})</span>
              </div>
            )}
            {showLogements && (
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-emerald-500" />
                <span className="text-slate-400">{t('map.logements')} ({logements.length})</span>
              </div>
            )}
            {showAutorisations && (
              <>
