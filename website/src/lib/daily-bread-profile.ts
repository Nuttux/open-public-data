 ...
 * aligned with the calculator on `/ville/paris/daily-bread`.
 *
 ...
 *  - etat : bucketMonthly = etatMonthly ; node share = mission.share_of_state_net.
...
export function projectLevel2Monthly(
  ...
  if (bucket === "etat") {
    return monthlies.etatMonthly * level2ShareOfParent;
  }
 ...
 * `daily_bread_drilldown.json` la publie.
 */
export function projectEtatAggregationMonthly(
  monthlies: DailyBreadProfileMonthlies,
  aggShareOfParent: number,
): number {
  return monthlies.etatMonthly * aggShareOfParent;
}
