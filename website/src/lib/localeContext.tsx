...
// To opt out for a specific page, wrap it in <LocaleProvider /> with a fixed
// override or just don't read t() in that scope.
...
function rewriteForCity(value: string, citySlug: string): string {
  ...
  // Capitalised first letter of adjectives for inclusive forms ("Parisien·ne").
  const adjMCap = target.adj_m.charAt(0).toUpperCase() + target.adj_m.slice(1);
  // Replace from longest patterns first to avoid partial matches.
  // Note: \b doesn't anchor across the U+00B7 middle dot, so we list the
  // inclusive-writing forms explicitly without \b.
  return value
