...
export async function generateMetadata({
  ...
  if (!entry) {
    return {
      title:
        locale === "en"
          ? "Department breakdown not found — France Open Data"
          : "Détail département introuvable — France Open Data",
      robots: { index: false },
    };
  }
  const label = locale === "en" ? entry.label_en : entry.label_fr;
  const title = `${label} — Département · Daily Bread · France Open Data`;
  const canonical = `/ville/paris/daily-bread/bucket/local/dept/${level2}`;
  return {
    title,
    alternates: {
      canonical,
...
export default async function StandaloneDeptPage({
  params,
  searchParams,
}: {
  params: Promise<Params>;
