...
export const alt = "France Open Data — Prélèvements obligatoires";
...
export default async function FiscaliteOG() {
  const data = loadEurostatFiscalite();
  const totPct = data?.fr_total_po.pc_gdp ?? 0;
  const totEur = data?.fr_total_po.mio_eur ?? 0;
      ...
      <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", background: "#faf9f5", padding: "64px 72px", fontFamily: "sans-serif" }}>
        ...
        <div style={{ display: "flex", gap: 56, alignItems: "flex-end" }}>
          <div style={{ display: "flex", flexDirection: "column" }}>
            <div style={{ display: "flex", fontSize: 13, color: "#666", letterSpacing: 2, textTransform: "uppercase" }}>Total France</div>
            <div style={{ display: "flex", fontSize: 110, fontWeight: 800, color: "#111", letterSpacing: -3, lineHeight: 1 }}>{fmt(totPct)} %</div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: 32, paddingTop: 16, borderTop: "2px solid #111", fontSize: 14, color: "#666", letterSpacing: 2, textTransform: "uppercase" }}>
          <div style={{ display: "flex" }}>Source unique Eurostat · gov_10a_taxag</div>
          <div style={{ display: "flex" }}>franceopendata.org/fiscalite</div>
        </div>
      </div>
    ),
    { ...size },
