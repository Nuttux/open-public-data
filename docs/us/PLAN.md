...
- [x] 1.5 Page `app/us/national/` (EN-only): receipts → functions daily-bread view composed from fusion primitives; per-resident scaling. Live 2026-07-16 — hero per-resident FYTD with completeness label, 9 receipt sources + 17 positive functions ranked (the 2 negative categories sit in a separate offsets block, never in the bars), BalanceStack deficit view, 1790→today debt chart (log scale, nominal-dollars caption). Provenance = visible per-section `Source:` links from the JSON source blocks; the DataProvenance modal is deferred — it reads `data_lineage.json` chart ids (file owned by `routes-v2`), wiring `dbt_us_*` lineage into it is follow-up.
