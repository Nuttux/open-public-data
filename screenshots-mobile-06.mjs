...
const OUT_DIR = join(__dirname, 'mockups/06-fusion/_mobile-previews');
...
for (const file of files) {
  ...
  try {
    ...
    // Top of page (first viewport)
    await page.screenshot({ path: join(OUT_DIR, `${base}-01-top.png`), type: 'png' });

    // Full page screenshot
    await page.screenshot({ path: join(OUT_DIR, `${base}-full.png`), type: 'png', fullPage: true });
